globals [
  infectedPop        ;; The number of infected people
  recoveredPop       ;; The number of recovered people
  timeStep           ;; The global length of the time step
]

turtles-own [
  infected?          ;; If true, the person is infected. 
  recovered?         ;; If true, the person is infected.
]
...
to setup-people
  crt initial-people
    [ setxy random-xcor random-ycor
      ...
      set infected? (who < initial-infected)
      set recovered? false
      set infectedPop initial-infected
      set recoveredPop 0
      set timeStep 1
      assign-color ]
end
...
to go
  ...
  ask turtles
    [ ifelse infected?
      [ if random-float 100 < recoveryRate * timeStep
        [ set recovered? true
          set infected? false
          set recoveredPop recoveredPop + 1
          set infectedPop infectedPop - 1]]
      [ if not recovered? 
        [ if random-float 100 < (infectionRate * infectedPop * timeStep) / initial-people
          [ set infected? true 
            set infectedPop infectedPop + 1] ]]]
...
to-report %infected
  report infectedPop / initial-people
end
@#$#@#$#@
GRAPHICS-WINDOW
288
...
PENS
"Susceptible" 1.0 0 -10899396 true "" "plot initial-people - infectedPop - recoveredPop"
"infected" 1.0 0 -2674135 true "" "plot infectedPop"
"recovered" 1.0 0 -13345367 true "" "plot recoveredPop"
